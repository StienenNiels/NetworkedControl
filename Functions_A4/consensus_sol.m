function [traj,xf,u,lambda,fval,exitflag,output,lam] = consensus_sol(Planes, phi, plotgen)
    ...
    while dxf > tol
        ...
        for i=1:4
            h = Planes(i).h;
            [u_sol,fval,exitflag,output,lam] = quadprog(Planes(i).H,h,Planes(i).A_u,Planes(i).b_u,Planes(i).A_eq,-Planes(i).b_eq+xf_con(dim.nx*(i-1)+1:dim.nx*i),[],[],[],opts);
